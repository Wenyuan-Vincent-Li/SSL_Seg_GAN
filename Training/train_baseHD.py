...
def train_single_scale(dataloader, netD, netG, netS, reals, Gs, Ss, in_s, in_s_S, NoiseAmp, NoiseAmpS, opt):
    ...
    for epoch in range(opt.niter):  # niter = 2000
        ...
        for j, data in enumerate(dataloader):
            ...
            # train with real
            netD.zero_grad()
            pred_real = netD(data['image'], data['label'][:,0,...])
            ...
            if (j == 0) & (epoch == 0):  # first iteration training in this level
                if Gs == [] and Ss == []:
                    ...
                    onehot_label = mask2onehot(data['label'][:,0,...], opt.label_nc)
            ...
            pred_fake = netD(fake.detach(), data['label'][:,0,...])  # output shape [1, 1, 16, 16] -> [1, 1, 23, 23]
