...
def train_single_scale(dataloader, netD, netG, netS, reals, Gs, Ss, in_s, in_s_S, NoiseAmp, NoiseAmpS, opt):
    ...
    for epoch in range(opt.niter):  # niter = 2000
        if Gs == [] and Ss == []:
            ...
            # seg_mask = functions.generate_noise([1, opt.nzx, opt.nzy], opt.batchSize)
            # seg_mask = seg_mask.expand(opt.batchSize, opt.label_nc, opt.nzx, opt.nzy)
        else:
        ...
        for j, data in enumerate(dataloader):
            ...
            if (j == 0) & (epoch == 0):  # first iteration training in this level
                if Gs == [] and Ss == []:
                    ...
                    in_s_S = prev_S
                    mask = data['label']
                    opt.noise_amp = opt.noise_amp_S = opt.noise_amp_init
                else:
            ...
            ###########################
            netS.zero_grad()
            pred_fake_S = netD(data['image'], segment_mask)
            ...
            errS = loss_G_GAN_S + loss_G_GAN_Feat_S
            errS.backward()
            optimizerS.step()
